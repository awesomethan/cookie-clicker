...
const cookieClickSound = new Audio();
cookieClickSound.src = "files/click.mp3";
...
function addCookie() {
  cookieClickSound.currentTime = 0;
  ...
  if (flag && numberOfCookies >= goals[index]) {
    ...
    if (numberOfCookies >= 2000) {
      congratsTextElement.innerText =
        "Wow! You reached goal 10! You are a pro at clicking cookies now :D";
      ...
      index++;
      congratsTextElement.innerText =
        "Congrats! You reached goal " + index + "!";
    }
  }
  cookieCountElement.innerText = "Cookies: " + numberOfCookies;
  if (flag) {
    upgradeTextElement.innerText =
      cookiesToUpgrade + " more cookies to upgrade!";
  }
}
